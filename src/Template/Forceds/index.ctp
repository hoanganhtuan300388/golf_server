                    ...
                    <div class="row">
                        <div class="col-md-4">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-7">
                                    <?= $this->Form->text('search.keyword', ['class' => 'form-control input-sm']) ?>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= $this->Form->label('version', __('version'), ['class' => 'col-sm-4 control-label']) ?>
                                <div class="col-sm-7">
                                    <?= $this->Form->text('search.version', ['class' => 'form-control input-sm', 'maxlength' => 20]) ?>
                                </div>
                            </div>
                            <div class="form-group">
                            </div>
                                ...
                                <table class="table table-bordered table-striped dataTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th width="100" class="text-center"><?= __('version') ?></th>
                                            <th class="text-center"><?= __('state') ?></th>
                                            <th class="text-center"><?= __('Action') ?></th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php if(count($forceds) > 0) { ?>
                                            <?php foreach($forceds as $key => $item) { ?>
                                                <tr>
                                                    <td><?= $item['id'] ?></td>
                                                    <td><?= $this->Html->link(h($item['title']), ['action' => 'view', $item['id']]) ?></td>
                                                    <td><?= nl2br(h($item['body'])) ?></td>
                                                    <td class="text-center"><?= h($item['version']) ?></td>
                                                    <td class="text-center"><?= $item['status'] == MAINTENANCE_STATUS_ENABLE ? __('Enabled') : __('Disabled') ?></td>
                                                    <td>
                                                        <?php if($item['status'] == MAINTENANCE_STATUS_ENABLE) { ?>
